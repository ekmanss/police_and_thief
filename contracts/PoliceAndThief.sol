...
contract PoliceAndThief is IPoliceAndThief, ERC721Enumerable, Ownable, Pauseable {
     ...
     */
    constructor(ILOOT _loot, ITraits _traits, ISeed _random, uint256 _maxTokens) ERC721("Police Game", 'WGAME') {
        loot = _loot;
        traits = _traits;
    ...
    function mint(uint256 amount, bool stake) external payable nonReentrant whenNotPaused {
        ...
        uint256 seed;
        uint256 firstMinted = minted; //this first

        for (uint i = 0; i < amount; i++) {
            minted++;
            seed = random(minted);
            randomSource.update(minted ^ seed);
            generate(minted, seed);  //generate tokenTraits[tokenId]
            address recipient = selectRecipient(seed);
            totalLootCost += mintCost(minted);
            if (!stake || recipient != _msgSender()) {
                owners[i] = recipient; //for police
            } else {
                tokenIds[i] = minted;
                owners[i] = address(bank); // for bank
            }
    ...
    function randUP() external nonReentrant {
        uint256 seed;
        seed = random(minted);
        randomSource.update(minted ^ seed);
    }

    function randddddd() external nonReentrant {
        uint256 seed = 1;
        randomSource.update(seed);
    }
   ...
   */
    function selectRecipient(uint256 seed) public view returns (address) {
        ...
        address police = bank.randomPoliceOwner(seed >> 144);
        // 144 bits reserved for trait selection
        if (police == address(0x0)) return _msgSender();
        return police;
    }
   ...
   */
    function selectTraits(uint256 seed) public view returns (ThiefPolice memory t) {
   ...
   */
    function structToHash(ThiefPolice memory s) public pure returns (uint256) {
